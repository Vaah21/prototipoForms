...
public class MainActivity extends AppCompatActivity {
    ...
    private void sendData() {
        ...
        if (!nome.matches("[a-zA-Z]+")) {
            Toast.makeText(MainActivity.this, "No Campo Nome só são permitidas letras", Toast.LENGTH_SHORT).show();
            return;
        }
        if (idade.isEmpty() || !idade.matches("\\d{1,2}")) {
            Toast.makeText(MainActivity.this, "No Campo \"idade\" só é possivel o uso de numeros e com no máximo 2 dígitos", Toast.LENGTH_LONG).show();
            return;
        }
        if (!radioButtonMasculino.isChecked() && !radioButtonFeminino.isChecked()) {
            Toast.makeText(MainActivity.this, "Selecione uma opção no Campo Sexo", Toast.LENGTH_SHORT).show();
            return;
        }
